...
app.get('/api/persons', (request, response, next) => {
	Person.find({}).then((documents) => response.json(documents))
		.catch((e) => next(e))
})
...
app.delete('/api/persons/:id', (request, response, next) => {
	...
	Person.deleteOne({ _id: id }).then((deletedDocument) => response.status(204).end()).catch((e) => next(e))
})

app.get('/api/info', (request, response) => {
	const peopleNumber = 0
